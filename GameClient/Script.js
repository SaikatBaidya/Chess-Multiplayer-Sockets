...
function handleDragStart(e) {
  ...
  e.dataTransfer.setData(
    "source",
    e.target.parentElement.dataset.position || "bin"
  );
...
async function pairMe() {
  ...
  const resp = await fetch(
    `http://localhost:11000/pairme?player=${encodeURIComponent(username)}`
  );
  const data = await resp.json();
  gameId = data.gameId;
  document.getElementById(
    "pairStatus"
  ).innerText = `Paired! Game ID: ${gameId}, State: ${data.state}, Player1: ${data.player1}, Player2: ${data.player2}`;
  pollOpponentMove();
}
...
async function sendMove(move) {
  try {
    const resp = await fetch(
      `http://localhost:11000/mymove?player=${encodeURIComponent(
        username
      )}&id=${encodeURIComponent(gameId)}&move=${encodeURIComponent(move)}`
    );
    const data = await resp.json();
    if (data.status === "ok") {
...
// Poll the server every 2 seconds for the opponent's move
...
async function pollOpponentMove() {
  ...
  try {
    const resp = await fetch(
      `http://localhost:11000/theirmove?player=${encodeURIComponent(
        username
  ...
  setTimeout(pollOpponentMove, 2000); // poll again in 2 seconds
}
...
async function quitGame() {
  ...
  try {
    const resp = await fetch(
      `http://localhost:11000/quit?player=${encodeURIComponent(
        username
      )}&id=${encodeURIComponent(gameId)}`
    );
    const data = await resp.json();
    if (data.status === "quit") {
      ...
      // Optionally, reset UI or reload page
      location.reload();
    } else {
